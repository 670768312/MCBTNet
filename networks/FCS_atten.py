...
from networks.bra_block_atten import Block
...
class FcsAttention(nn.Module):
    def __init__(self, in_channels, out_channels, img_size, reduction=16):
        ...
        c2wh = dict([(192, img_size//4), (384, img_size//8), (768, img_size//16)])
...
class BRAUnetSystem(nn.Module):
    def __init__(self, img_size=256,depth=[3, 4, 8, 3],depths_decoder=[2,2,2,2], in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        self.FCS1 = FcsAttention(in_channels=embed_dim[1], out_channels=embed_dim[1], img_size=img_size)
        self.FCS2 = FcsAttention(in_channels=embed_dim[2], out_channels=embed_dim[2], img_size=img_size)
        self.FCS3 = FcsAttention(in_channels=embed_dim[3], out_channels=embed_dim[3], img_size=img_size)
    ...
    def up_x4(self, x):
        ...
        return x
    def forward(self, x):
        global_attenmap.initialize()
        x, x_downsample = self.forward_features(x)
        x = self.forward_up_features(x, x_downsample)
        x = self.up_x4(x)
        return x, global_attenmap.get_value()[0], global_attenmap.get_value()[1]
    def flops(self):
        flops = 0
