...
class DualAttention(nn.Module):
    def __init__(self, in_channels, reduction_ratio=2,out_channels=None):
        ...
        self.channel_se = nn.Sequential(
            ...
            nn.Conv2d(self.in_channels, self.in_channels // self.reduction_ratio, kernel_size=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(self.in_channels // self.reduction_ratio, self.in_channels, kernel_size=1),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        # Apply Channel Squeeze and Excitation
        channel_attention = self.channel_se(x)
        channel_refined = x * channel_attention

        # Apply Spatial Squeeze and Excitation
        spatial_attention = self.spatial_se(x)
        spatial_refined = x * spatial_attention

        # Combining both attentions
        output = channel_refined + spatial_refined
        return output
...
class BRAUnetSystem(nn.Module):
    def __init__(self, img_size=256,depth=[3, 4, 8, 3],depths_decoder=[2,2,2,2], in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        self.layers_to_fusion = layers_to_fusion

        self.dualattention1 = DualAttention(in_channels=embed_dim[1],out_channels=embed_dim[1])
        self.dualattention2 = DualAttention(in_channels=embed_dim[2],out_channels=embed_dim[2])
        self.dualattention3 = DualAttention(in_channels=embed_dim[3],out_channels=embed_dim[3])
        self.dualattention_multifusion = DualAttention(in_channels=len(layers_to_fusion) * embed_dim[0], out_channels=len(layers_to_fusion) * embed_dim[0])
        ...
        self.FinalConcatDimBack = nn.Linear(len(layers_to_fusion) * embed_dim[0],
                                  embed_dim[0])
    ...
    def tensor_BLC_2_BCHW(self,x):  # [B,L,C]--→[B,C,H,W]  L=H*W
        B, L, C = x.shape
    ...
    def MultiScaleFusion(self, x_upsample):
        for i, layer_up in enumerate(self.layers_up2fusion[1:], start=1):
            if i == 1:
                x_upsample[i - 1] = self.tensor_BLC_2_BCHW(x_upsample[i - 1])
                x_upsample[i - 1] = layer_up(x_upsample[i - 1])
            elif i == 2:
                x_upsample[i - 2] = self.tensor_BLC_2_BCHW(x_upsample[i - 2])
                x_upsample[i - 2] = layer_up(x_upsample[i - 2])
                x_upsample[i - 1] = self.tensor_BLC_2_BCHW(x_upsample[i - 1])
                x_upsample[i - 1] = layer_up(x_upsample[i - 1])

        tensors_to_merge = [x_upsample[idx] for idx in self.layers_to_fusion]
        # 沿着最后一个维度拼接张量
        x = torch.cat(tensors_to_merge, dim=-1)  # B L C

        x = self.tensor_BLC_2_BCHW(x)  # B C H W
        x = self.dualattention_multifusion(x)  # B C H W
