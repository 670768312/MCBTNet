...
class TopkRouting(nn.Module):
    ...
    def forward(self, query:Tensor, key:Tensor)->Tuple[Tensor]:
        ...
        print(f"Top-k index dimensions: {topk_index.shape}")  # 查看选择的Top-k区域的维度
        print(f"Top-k logit weights dimensions: {topk_attn_logit.shape}")  # 查看Top-k区域的权重的维度
...
class BiLevelRoutingAttention(nn.Module):
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        if self.auto_pad:
            ...
            print(f"\nEntering BRA computation:")
            print(f"Initial input dimensions (H, W, C): ({H}, {W}, {C})")
        ...
        print(f"Total regions after splitting (n_win x n_win): {H // self.n_win} x {W // self.n_win}")
        print(f"Dimensions of each region (H, W, C): ({self.n_win}, {self.n_win}, {C})")
        ...
        print(f"KV pixel count and dimensions (H, W, C) after downsampling:")
        print(f"Number of KV regions: {kv_pix.size(1)}")
        print(f"Dimensions of each KV region: (H={self.n_win}, W={self.n_win}, C={kv_pix.size(-1)})")
