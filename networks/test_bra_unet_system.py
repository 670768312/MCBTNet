...
class BRAUnetSystem(nn.Module):
    ...
    def forward_features(self, x):  #对输入x进行三次下采样(downsample_layers)，
        ...
        for i in range(3):
            ...
            print(f"Downsample Layer {i + 1} output shape: {x.shape}")
        ...
        print(f"Downsample Layer 4 output shape: {x.shape}")
    ...
    def forward_up_features(self, x, x_downsample):
        ...
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                ...
                B, L, C = x.shape
                print(f"beforesccsa {4 - inx} output shape: {x.shape}")
                x = x.view(B, int(math.sqrt(L)), int(math.sqrt(L)), C)
                x = x.permute(0, 3, 1, 2)
                x = self.sccsa3(x)
                print(f"aftersccsa {4 - inx} output shape: {x.shape}")
                x = x.flatten(2).transpose(1, 2)
                ...
                x = layer_up(x)
                x_upsample.append(x)
            print(f"Layer {inx + 1} output shape: {x.shape}")
        x = self.norm_up(x)  # B L C
        return x
    def up_x4(self, x):
        H, W = self.patches_resolution
        B, L, C = x.shape
        print(f"beforex4 shape: {x.shape}")
        assert L == H * W, "input features has wrong size"
        if self.final_upsample == "expand_first":
            x = self.up4(x)  #高宽各乘4 通道数不变
            print(f"afterx4 shape: {x.shape}")
            x = x.view(B, 4 * H, 4 * W, -1)  #[1, 50176, 96]➡[1, 224, 224, 96]  将高和宽拆开
            x = x.permute(0, 3, 1, 2)  # B,C,H,W
            x = self.output(x)  #通道数变为num_class,宽高变为初始输入的image_size
            print(f"Final Upsample output shape: {x.shape}")
        return x
    def forward(self, x):
